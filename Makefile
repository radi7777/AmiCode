...
CFLAGS   = -m68030 -mcrt=nix20 -Os -Wall -Wno-pointer-sign -MMD -MP -Iinclude -Ivendor/mui/include
LDFLAGS  = -m68030 -mcrt=nix20 -s
# Stack-Tausch aus libnix erzwingen (siehe __stack in main.c/gui.c)
LDLIBS   = -Wl,-u,___stkswap

CORE = config json net provider_openai tools shell agent ui toolchain web skills
CLI  = main $(CORE)
# muistubs.c MUSS eine eigene Uebersetzungseinheit bleiben (siehe Kopfkommentar)
GUI  = gui gui_agent gui_prefs gui_newproj gui_projsel gui_sessions gui_skills muistubs $(CORE)

CLI_OBJS = $(CLI:%=build/%.o)
GUI_OBJS = $(GUI:%=build/%.o)

all: build/amicode build/AmiCodeIDE build/AmiCodeIDE.info
